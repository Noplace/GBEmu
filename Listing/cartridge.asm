...
CONST	ENDS
CONST	SEGMENT
$SG129638 DB	'Code\emulation\gb\cartridge.cpp', 00H
$SG129643 DB	'Code\emulation\gb\cartridge.cpp', 00H
$SG129648 DB	'Code\emulation\gb\cartridge.cpp', 00H
CONST	ENDS
...
; 165  :   void Write(uint16_t address, uint8_t data) {
...
; 166  :    if (address >= 0x0000 && address <= 0x1FFF) {
...
; 168  : 	  } else if (address >= 0x2000 && address <= 0x3FFF) {
...
; 171  :         rom_bank_number = 1;

	mov	ecx, DWORD PTR _this$[ebp]
	mov	BYTE PTR [ecx+16], 1
$LN10@Write:

; 172  :     } else if (address >= 0x4000 && address <= 0x5FFF) {
...
; 174  :         rom_bank_number |= (data&0x3)<<5;
...
; 180  :     } else if (address >= 0x6000 && address <= 0x7FFF) {
...
; 182  : 
; 183  :     } else if (address >= 0xA000 && address <= 0xBFFF) {
...
; 152  :   uint8_t Read(uint16_t address) {
...
; 153  :     if (address >= 0x0000 && address <= 0x3FFF) {
...
; 154  : 	    return cartridge->rom()[address];
...
; 155  : 	  } else if (address >= 0x4000 && address <= 0x7FFF) {
...
; 156  :       return cartridge->rom()[address+0x4000*(rom_bank_number-1)];
...
; 157  :     } else if (address >= 0xA000 && address <= 0xBFFF) {
...
; 159  :         return eram_[(address&0x1FFF)+(0x2000*ram_bank_number*mode)];
...
; 150  :     MemoryBankController::Deinitialize();
...
; 141  :   void Initialize(Cartridge* cartridge) {
...
; 142  :     MemoryBankController::Initialize(cartridge);
...
; 145  :     rom_bank_number = 1;

	mov	eax, DWORD PTR _this$[ebp]
	mov	BYTE PTR [eax+16], 1

; 146  :     ram_bank_number = 0;
...
; 124  :   uint8_t Read(uint16_t address) {
...
; 125  :       if (address >= 0x0000 && address <= 0x3FFF) {
...
; 126  : 	    return cartridge->rom()[address];
...
; 127  : 	  } else if (address >= 0x4000 && address <= 0x7FFF) {
...
; 128  :       return cartridge->rom()[address];
...
; 129  :     } else if (address >= 0xA000 && address <= 0xBFFF) {
...
; 122  :     MemoryBankController::Deinitialize();
...
; 47   : void Cartridge::Write(uint16_t address, uint8_t data) {
...
; 43   : uint8_t Cartridge::Read(uint16_t address) {
...
; 19   : void Cartridge::ReadFile(const char* filename, CartridgeHeader* header) {
...
; 20   :   core::io::DestroyFileBuffer(&rom_);
...
; 21   :   uint8_t* data;
; 22   :   size_t length;
; 23   :   core::io::ReadWholeFileBinary(filename,&data,length);
...
; 24   :   memcpy(header,data+0x100,0x50);
...
; 28   :   }
; 29   :   rom_ = new uint8_t[header->rom_size_bytes()];

	push	29					; 0000001dH
	push	OFFSET $SG129638
	push	1
	mov	ecx, DWORD PTR _header$[ebp]
	call	?rom_size_bytes@CartridgeHeader@gb@emulation@@QAEIXZ ; emulation::gb::CartridgeHeader::rom_size_bytes
	push	eax
...
; 30   :   memcpy(rom_,data,header->rom_size_bytes());
...
; 31   :   this->header = (CartridgeHeader*)&rom_[0x100];
...
; 34   :   } else if (header->cartridge_type == 1) {
...
; 38   :   core::io::DestroyFileBuffer(&data);
...
; 12   : void Cartridge::Deinitialize() {
...
; 13   :   core::io::DestroyFileBuffer(&rom_);
...
; 6    : void Cartridge::Initialize(Emu* emu) {
