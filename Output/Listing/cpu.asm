...
EXTRN	?Check@Timer@gb@emulation@@QAEXXZ:PROC		; emulation::gb::Timer::Check
...
; 675  :       reg.raw8[reg_index[code&0x7]] = r;
...
; 666  :       return reg.raw8[reg_index[code&0x7]];
...
; 464  :   if (mode == 10) { //dest,d8

	xor	eax, eax
	je	SHORT $LN11@LD

; 465  : 		reg.raw8[dest] = mem_->Read8(reg.PC++);
...
; 466  : 	}else if (mode == 11) { //(dest),d8

	xor	edx, edx
	je	SHORT $LN9@LD

; 467  : 		auto d8 = mem_->Read8(reg.PC++);
...
; 468  : 		mem_->Write8(reg.raw16[dest],d8);
...
; 469  : 	} else if (mode == 12) { //0xFF00+d8 src

	xor	eax, eax
	je	SHORT $LN7@LD

; 470  :     uint8_t a8 = mem_->Read8(reg.PC++);
...
; 471  : 		mem_->Write8(0xFF00+a8,reg.raw8[src]);
...
; 472  : 	} else if (mode == 13) { //dest,0xFF00+d8 

	xor	ecx, ecx
	je	SHORT $LN5@LD

; 473  :     uint8_t a8 = mem_->Read8(reg.PC++);
...
; 474  : 		reg.raw8[dest] = mem_->Read8(0xFF00+a8);
...
; 475  : 	} else if (mode == 14) { //(d16),src

	xor	edx, edx
	je	$LN3@LD

; 476  : 		uint16_t d16 = mem_->Read8(reg.PC++);
...
; 478  :     mem_->Write8(d16,reg.raw8[src]);
...
; 479  : 	} else if (mode == 15) { //src,(d16)

	mov	ecx, 1
	test	ecx, ecx
	je	$LN12@LD

; 480  : 		uint16_t d16 = mem_->Read8(reg.PC++);
...
; 482  :     reg.raw8[dest] = mem_->Read8(d16);
...
; 628  :   arithmeticMode<dest,src,mode>(a,b);
...
; 630  :   updateCpuFlagZ(reg.raw8[dest]);
...
; 425  : 	reg.raw8[dest] = mem_->Read8(0xFF00+reg.raw8[src]);
...
; 929  :   simulateSpriteBug();

	mov	ecx, DWORD PTR _this$[ebp]
	call	?simulateSpriteBug@Cpu@gb@emulation@@AAEXXZ ; emulation::gb::Cpu::simulateSpriteBug

; 930  :   reg.raw16[dest] = pop();
...
; 619  :   arithmeticMode<dest,src,mode>(a,b);
...
; 621  :   updateCpuFlagZ(reg.raw8[dest]);
...
; 608  :   arithmeticMode<dest,src,mode>(a,b);
...
; 611  :   updateCpuFlagZ(reg.raw8[dest]);
...
; 420  : 	mem_->Write8(0xFF00+reg.raw8[dest],reg.raw8[src]);
...
; 592  :   arithmeticMode<dest,src,mode>(a,b);
...
; 597  :     updateCpuFlagC(a-carry,b,1);
...
; 600  :     updateCpuFlagH(a-carry,b,1);
...
; 601  :   updateCpuFlagZ(reg.raw8[dest]);
...
; 887  :   int table[2] = {((reg.F.raw & (1<<condbit))>>condbit),
...
; 888  :     ~((reg.F.raw & (1<<condbit))>>condbit)};
...
; 858  :   int table[2] = {((reg.F.raw & (1<<condbit))>>condbit),
...
; 859  :     ~((reg.F.raw & (1<<condbit))>>condbit)};
...
; 907  :   int table[2] = {((reg.F.raw & (1<<condbit))>>condbit),
...
; 908  :     ~((reg.F.raw & (1<<condbit))>>condbit)};
...
; 581  :   arithmeticMode<dest,src,mode>(a,b);
...
; 585  :   updateCpuFlagZ(reg.raw8[dest]);
...
; 556  :   arithmeticMode<dest,src,mode>(a,b);
...
; 562  :     updateCpuFlagC(a+carry,b,0);
...
; 565  :     updateCpuFlagH(a+carry,b,0);
...
; 566  :   updateCpuFlagZ(reg.raw8[dest]);
...
; 519  :   arithmeticMode<dest,src,mode>(a,b);
...
; 523  :   updateCpuFlagZ(reg.raw8[dest]);
...
; 405  : 	reg.raw8[dest] = reg.raw8[src];
...
; 415  : 	reg.raw8[dest] = mem_->Read8(reg.raw16[src]);
...
; 410  : 	mem_->Write8(reg.raw16[dest],reg.raw8[src]);
...
; 817  :     //updateCpuFlagH(reg.raw8[dest],1,1);
; 818  :     --reg.raw8[dest];
...
; 823  :     updateCpuFlagZ(reg.raw8[dest]);
...
; 825  :     uint8_t data = mem_->Read8(reg.HL);
...
; 791  :     updateCpuFlagH(reg.raw8[dest],1,0);
...
; 457  :   simulateSpriteBug();

	mov	ecx, DWORD PTR _this$[ebp]
	call	?simulateSpriteBug@Cpu@gb@emulation@@AAEXXZ ; emulation::gb::Cpu::simulateSpriteBug

; 458  :   reg.raw8[dest] = mem_->Read8(reg.raw16[src]);
...
; 533  :   uint16_t r1 = (a&0xFFF) + (b&0xFFF);
...
; 535  :   uint32_t r2 = (a&0xFFFF) + (b&0xFFFF);
...
; 775  :   int table[2] = {((reg.F.raw & (1<<condbit))>>condbit),
...
; 776  :     ~((reg.F.raw & (1<<condbit))>>condbit)};
...
; 781  :     int8_t disp8 = mem_->Read8(reg.PC++);
...
; 444  : 	mem_->Write8(reg.raw16[dest],reg.raw8[src]);
...
; 430  : 	reg.raw16[dest] = mem_->Read8(reg.PC++);
...
; 431  : 	reg.raw16[dest] |= (mem_->Read8(reg.PC++))<<8;
...
; 432  :   if (dest != RegAF && (reg.raw16[dest]>=0xFE00&&reg.raw16[dest]<=0xFEFF) && emu_->lcd_driver()->lcdc().lcd_enable == 1)
...
; 450  :   simulateSpriteBug();

	mov	ecx, DWORD PTR _this$[ebp]
	call	?simulateSpriteBug@Cpu@gb@emulation@@AAEXXZ ; emulation::gb::Cpu::simulateSpriteBug

; 451  :   reg.raw8[dest] = mem_->Read8(reg.raw16[src]);
...
; 438  : 	mem_->Write8(reg.raw16[dest],reg.raw8[src]);
...
; 995  :   CP(reg.A,mem_->Read8(reg.HL));
...
; 991  :   CP(reg.A,mem_->Read8(reg.PC++));
...
; 978  : void Cpu::CP(uint8_t a, uint8_t b) {
...
; 768  : 	 int8_t disp8 = mem_->Read8(reg.PC++);
...
; 662  : 	uint8_t code = emu_->memory()->Read8(reg.PC++);
...
; 683  :     uint8_t bitshift = (code&0x38) >> 3;
...
; 684  :     reg.F.Z = (~(((test&(1<<bitshift))>>bitshift))&0x1);
...
; 687  : 
; 688  :   } else if ((code & 0xC0) == 0x80) { //res
...
; 689  :     uint8_t test = getr();

	lea	ecx, DWORD PTR _getr$[ebp]
	call	??R<lambda_cf4d431a50c7ddf749029b7e3fcc8a53>@@QBEEXZ ; <lambda_cf4d431a50c7ddf749029b7e3fcc8a53>::operator()
	mov	BYTE PTR _test$16[ebp], al

; 690  :     uint8_t bitshift = (code&0x38) >> 3;
...
; 693  :   } else if ((code & 0xC0) == 0xC0) { //set
...
; 694  :     uint8_t test = getr();

	lea	ecx, DWORD PTR _getr$[ebp]
	call	??R<lambda_cf4d431a50c7ddf749029b7e3fcc8a53>@@QBEEXZ ; <lambda_cf4d431a50c7ddf749029b7e3fcc8a53>::operator()
	mov	BYTE PTR _test$14[ebp], al

; 695  :     uint8_t bitshift = (code&0x38) >> 3;
...
; 698  :   } else  if ((code & 0xF8) == 0x30) { //swap
...
; 714  :   } else if ((code&0xF8) == 0x08) { //RRC r
...
; 722  :   } else if ((code&0xF8) == 0x10) { //RL r
...
; 731  :   } else if ((code&0xF8) == 0x18) { //RR r
...
; 740  : 
; 741  :   } else if ((code&0xF8) == 0x20) { //SLA r
...
; 749  :   } else if ((code&0xF8) == 0x28) { //SRA r
...
; 756  :   } else if ((code & 0xF8) == 0x38) { //SRL n
...
; 542  : 	uint16_t a = reg.SP;

	mov	eax, DWORD PTR _this$[ebp]
	mov	cx, WORD PTR [eax+1072]
	mov	WORD PTR _a$[ebp], cx

; 543  : 	int8_t r8 = mem_->Read8(reg.PC++);
...
; 545  : 
; 546  :   updateCpuFlagC(a&0xFF,r8,0);
...
; 547  :   updateCpuFlagH(a&0xFF,r8,0);
...
; 509  :   uint16_t a16 = mem_->Read8(reg.PC++);
...
; 511  :   mem_->Write8(a16,reg.SP&0xFF);
...
; 512  :   mem_->Write8(a16+1,reg.SP>>8);
...
; 496  : 	uint16_t a = reg.SP;

	mov	eax, DWORD PTR _this$[ebp]
	mov	cx, WORD PTR [eax+1072]
	mov	WORD PTR _a$[ebp], cx

; 497  : 	int8_t r8 = mem_->Read8(reg.PC++);
...
; 499  :   updateCpuFlagC(a&0xFF,r8,0);
...
; 500  :   updateCpuFlagH(a&0xFF,r8,0);
...
; 501  :   /*uint16_t r1 = (a&0xFFF) + (r8&0xFFF);
; 502  :   reg.F.H = r1>0xFFF?1:0;
; 503  :   uint32_t r2 = (a&0xFFFF) + (r8&0xFFFF);
; 504  :   reg.F.C = r2>0xFFFF?1:0;*/
; 505  : 	Tick();Tick();Tick();Tick();
...
; 1041 : void Cpu::simulateSpriteBug() {
...
; 1044 :     emu_->lcd_driver()->sprite_bug_counter = 80;//20cycles
...
; 1045 :     //auto oam = emu_->memory()->oam();
; 1046 :     //for (int i=8;i<0xA0;++i)
; 1047 :     //  oam[i] = rand()&0xFF;
; 1048 :     sprite_bug = 0;
...
; 342  :   //reg.F._unused = 0;//always 0 according to docs
; 343  :   //StopAt(0xC47A);
; 344  :   //StopAt(0x0100);
; 345  :   //StopAt(0x0073);
; 346  :   if (cpumode_ == CpuModeStop) {
...
; 349  :   }
; 350  :   if (cpumode_ == CpuModeNormal) {

	mov	ecx, DWORD PTR _this$[ebp]
	cmp	DWORD PTR [ecx+24], 0
	jne	$LN15@Step

; 351  :     if (sprite_bug!=0) --sprite_bug;
...
; 352  :     opcode = emu_->memory()->Read8(reg.PC++);
...
; 354  :   } else if (cpumode_ == CpuModeHalt) {
...
; 356  :   }
; 357  : 	emu_->timer()->Check();

	mov	ecx, DWORD PTR _this$[ebp]
	mov	ecx, DWORD PTR [ecx+4]
	call	?timer@Emu@gb@emulation@@QAEPAVTimer@23@XZ ; emulation::gb::Emu::timer
	mov	ecx, eax
	call	?Check@Timer@gb@emulation@@QAEXXZ	; emulation::gb::Timer::Check

; 358  : 
; 359  :   if (ime) {

	mov	edx, DWORD PTR _this$[ebp]
	movzx	eax, BYTE PTR [edx+1080]
	test	eax, eax
	je	$LN1@Step
...
; 362  : 			ime = false;
	...
	mov	edx, DWORD PTR _this$[ebp]
	mov	BYTE PTR [edx+1080], 0

; 363  :       cpumode_ = CpuModeNormal;
...
; 367  : 				emu_->memory()->interrupt_flag() &= ~0x1;
...
; 368  : 			} else if (test & 0x2) {

	movzx	edx, BYTE PTR _test$1[ebp]
	and	edx, 2
	je	SHORT $LN7@Step

; 369  : 				reg.PC = 0x0048; //lcdc status

	mov	eax, 72					; 00000048H
	mov	ecx, DWORD PTR _this$[ebp]
	mov	WORD PTR [ecx+1074], ax

; 370  : 				emu_->memory()->interrupt_flag() &= ~0x2;
...
; 371  : 			} else if (test & 0x4) {

	movzx	eax, BYTE PTR _test$1[ebp]
	and	eax, 4
	je	SHORT $LN5@Step

; 372  : 				reg.PC = 0x0050; //timer overflow

	mov	ecx, 80					; 00000050H
	mov	edx, DWORD PTR _this$[ebp]
	mov	WORD PTR [edx+1074], cx

; 373  : 				emu_->memory()->interrupt_flag() &= ~0x4;
...
; 374  : 			} else if (test & 0x8) {

	movzx	ecx, BYTE PTR _test$1[ebp]
	and	ecx, 8
	je	SHORT $LN3@Step

; 375  : 				reg.PC = 0x0058; //serial transfer

	mov	edx, 88					; 00000058H
	mov	eax, DWORD PTR _this$[ebp]
	mov	WORD PTR [eax+1074], dx

; 376  : 				emu_->memory()->interrupt_flag() &= ~0x8;
...
; 377  : 			} else if (test & 0x10) {

	movzx	edx, BYTE PTR _test$1[ebp]
	and	edx, 16					; 00000010H
	je	SHORT $LN1@Step

; 378  : 				reg.PC = 0x0060; //hi-lo p10-p13

	mov	eax, 96					; 00000060H
	mov	ecx, DWORD PTR _this$[ebp]
	mov	WORD PTR [ecx+1074], ax

; 379  : 				emu_->memory()->interrupt_flag() &= ~0x10;
