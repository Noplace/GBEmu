; Listing generated by Microsoft (R) Optimizing Compiler Version 17.00.60610.1 
	...
	TITLE	C:\Users\Khalid\Documents\GitHub\GBEmu\Solution\Code\winmain.cpp
	.686P
	.XMM
	include listing.inc
	.model	flat
...
; 28   :  
; 29   :     // Start by reserving large blocks of address space, and then
; 30   :     // gradually reduce the size in order to capture all of the
; 31   :     // fragments. Technically we should continue down to 64 KB but
; 32   :     // stopping at 1 MB is sufficient to keep most allocators out.
; 33   :  
; 34   :     const size_t LOW_MEM_LINE = 0x100000000LL;
; 35   :     size_t totalReservation = 0;
; 36   :     size_t numVAllocs = 0;
; 37   :     size_t numHeapAllocs = 0;
; 38   :     size_t oneMB = 1024 * 1024;
; 39   :     for (size_t size = 256 * oneMB; size >= oneMB; size /= 2)
; 40   :     {
; 41   :         for (;;)
; 42   :         {
; 43   :             void* p = VirtualAlloc(0, size, MEM_RESERVE, PAGE_NOACCESS);
; 44   :             if (!p)
; 45   :                 break;
; 46   :  
; 47   :             if ((size_t)p >= LOW_MEM_LINE)
; 48   :             {
; 49   :                 // We don't need this memory, so release it completely.
; 50   :                 VirtualFree(p, 0, MEM_RELEASE);
; 51   :                 break;
; 52   :             }
; 53   :  
; 54   :             totalReservation += size;
; 55   :             ++numVAllocs;
; 56   :         }
; 57   :     }
; 58   :  
; 59   :     // Now repeat the same process but making heap allocations, to use up
; 60   :     // the already reserved heap blocks that are below the 4 GB line.
; 61   :     HANDLE heap = GetProcessHeap();
; 62   :     for (size_t blockSize = 64 * 1024; blockSize >= 16; blockSize /= 2)
; 63   :     {
; 64   :         for (;;)
; 65   :         {
; 66   :             void* p = HeapAlloc(heap, 0, blockSize);
; 67   :             if (!p)
; 68   :                 break;
; 69   :  
; 70   :             if ((size_t)p >= LOW_MEM_LINE)
; 71   :             {
; 72   :                 // We don't need this memory, so release it completely.
; 73   :                 HeapFree(heap, 0, p);
; 74   :                 break;
; 75   :             }
; 76   :  
...
; 90   :                 break;
; 91   :  
; 92   :             if ((size_t)p >= LOW_MEM_LINE)
; 93   :             {
; 94   :                 // We don't need this memory, so release it completely.
; 95   :                 free(p);
; 96   :                 break;
; 97   :             }
; 98   :  
; 99   :             totalReservation += blockSize;
; 100  :             ++numHeapAllocs;
; 101  :         }
; 102  :     }
; 103  :  
; 104  :     // Print diagnostics showing how many allocations we had to make in
; 105  :     // order to reserve all of low memory, typically less than 200.
; 106  :     char buffer[1000];
; 107  :     sprintf_s(buffer, "Reserved %1.3f MB (%d vallocs,"
; 108  :                       "%d heap allocs) of low-memory.\n",
; 109  :             totalReservation / (1024 * 1024.0),
; 110  :             (int)numVAllocs, (int)numHeapAllocs);
; 111  :     OutputDebugStringA(buffer);
; 112  : #endif
; 113  : }
...
; 116  : int __stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd) {
	...
	call	__chkstk
	push	edi
	lea	edi, DWORD PTR [ebp-4240]
	mov	ecx, 1060				; 00000424H
...
; 117  :   ReserveBottomMemory();

	call	?ReserveBottomMemory@@YAXXZ		; ReserveBottomMemory

; 118  :   
; 119  :   app::Application app(hInstance,lpCmdLine,nShowCmd);
