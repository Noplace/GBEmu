; Listing generated by Microsoft (R) Optimizing Compiler Version 17.00.60610.1 
	...
	TITLE	C:\Users\Khalid\Documents\GitHub\GBEmu\Solution\Code\audio\output\directsound.cpp
	.686P
	.XMM
	include listing.inc
	.model	flat
...
_dutycycletable DB 00H
	...
	DB	01H
	DB	01H
$SG161051 DB	'dsound init', 0aH, 00H
	ORG $+3
$SG161063 DB	'dsound deinit', 0aH, 00H
CONST	ENDS
...
; File c:\users\khalid\documents\github\gbemu\solution\code\audio\output\directsound.cpp
_TEXT	SEGMENT
_v$1 = -12						; size = 4
_i$2 = -8						; size = 4
_dests$ = -4						; size = 4
...
; 269  :   int16_t *dests = (int16_t *)dest;

	mov	eax, DWORD PTR _dest$[ebp]
	mov	DWORD PTR _dests$[ebp], eax

; 270  : 
; 271  :   for (int32_t i=0; i < nsamples; i++)
...
; 52   :     static Application* Current() {

	push	ebp
	mov	ebp, esp

; 53   :       return current_app_;
...
; 49   :     DisplayWindow& display_window() {
...
; 281  : int DirectSound::EndWrite(void* data_pointer) {
...
; 282  : 
; 283  :   /*auto dest_buf=(uint8_t*)buf1;
; 284  :   auto dw=len1;
; 285  : 
; 286  :   auto src_buf=(float*)data_pointer;
; 287  :   while (dw) { 
; 288  :     float v = *src_buf++;
; 289  :     v *= 32768.0f; // * vol;
; 290  :     if (v >  32600.0f) v =  32600.0f;
; 291  :     if (v < -32600.0f) v = -32600.0f;
; 292  :     *dest_buf++ = (short)v;
; 293  :     dw--;
; 294  :   }
; 295  : 
; 296  :   if(buf2)  {
; 297  :     dest_buf=(uint8_t*)buf2;
; 298  :     dw=len2;
; 299  :     while(dw) {
; 300  :       float v = *src_buf++;
; 301  :       v *= 32768.0f; // * vol;
; 302  :       if (v >  32600.0f) v =  32600.0f;
; 303  :       if (v < -32600.0f) v = -32600.0f;
; 304  :       *dest_buf++ = (short)v;
; 305  :     dw--;
; 306  :     }
; 307  :   }*/
; 308  : 
...
; 310  :       clamp(buf1, (float*)data_pointer, len1/2);
...
; 312  :       clamp(buf2, (float*)data_pointer + len1/2, len2/2);
...
; 313  : 
; 314  :   if (secondary_buffer->Unlock(buf1, len1, buf2, len2)==DS_OK)
...
; 232  : int DirectSound::BeginWrite(uint32_t& samples) {
...
; 233  :    DWORD curpos;
; 234  :   int32_t nwrite = 0;

	mov	DWORD PTR _nwrite$[ebp], 0
$LN9@BeginWrite:

; 235  :   for (;;) {
; 236  :     HRESULT hr = secondary_buffer->GetCurrentPosition(&curpos, 0);
...
; 238  :     {
; 239  :       // find out how many bytes to write
; 240  :       curpos &= ~31u;
...
; 243  : 
; 244  :       nwrite = curpos - last_cursor_pos;
...
; 247  : 
; 248  :       hr = secondary_buffer->Lock(last_cursor_pos, nwrite, &buf1, &len1, &buf2, &len2, 0);
...
; 258  : 
; 259  :   // we got the lock
; 260  :   last_cursor_pos = curpos;

	mov	eax, DWORD PTR _this$[ebp]
	mov	ecx, DWORD PTR _curpos$[ebp]
	mov	DWORD PTR [eax+60], ecx

; 261  :   //g_dsound.bufcnt += nwrite;
; 262  :   samples = nwrite / 4;
...
; 144  : int DirectSound::Write(void* data_pointer, uint32_t size_bytes) {
...
; 164  : 
; 165  :   auto dest_buf=(uint8_t*)buf_ptr1;
...
; 167  : 
; 168  :   auto src_buf=(uint8_t*)data_pointer;
...
; 174  :     while(len) {*dest_buf++=*src_buf++;len--;}
...
; 175  :   }
; 176  : 
; 177  :   secondary_buffer->Unlock(buf_ptr1,buf_size1,buf_ptr2,buf_size2);
...
; 139  : void DirectSound::GetCursors(uint32_t& play, uint32_t& write) {
...
; 140  :   secondary_buffer->GetCurrentPosition((LPDWORD)&play,(LPDWORD)&write);
...
; 126  : uint32_t DirectSound::GetBytesBuffered() {
...
; 127  :  unsigned long cplay,cwrite;
; 128  : 
; 129  :  if(last_write_cursor==0xffffffff) return 0;
...
; 130  : 
; 131  :  secondary_buffer->GetCurrentPosition(&cplay,&cwrite);
...
; 132  : 
; 133  :  if(cplay>buffer_size_) return buffer_size_;
...
; 134  : 
; 135  :  if(cplay<last_write_cursor) return last_write_cursor-cplay;
...
; 136  :  return (buffer_size_-cplay)+last_write_cursor;
...
; 115  :   if (secondary_buffer != nullptr) {

	mov	eax, DWORD PTR _this$[ebp]
	cmp	DWORD PTR [eax+40], 0
	je	SHORT $LN4@Stop

; 116  :     auto hr = secondary_buffer->Stop();
...
; 118  :   }
; 119  :   if (primary_buffer != nullptr) {

	mov	ecx, DWORD PTR _this$[ebp]
	cmp	DWORD PTR [ecx+36], 0
	je	SHORT $LN2@Stop

; 120  :     auto hr = primary_buffer->Stop();
...
; 121  :     if (FAILED(hr)) return S_FALSE;
...
; 106  :   auto hr = primary_buffer->Play(0,0,DSBPLAY_LOOPING);
...
; 108  :   hr = secondary_buffer->Play(0,0,DSBPLAY_LOOPING);
...
; 109  :   if (FAILED(hr)) return S_FALSE;
...
; 110  :   hr = secondary_buffer->SetCurrentPosition(0);
...
; 101  :   OutputDebugString("dsound deinit\n");

	mov	esi, esp
	push	OFFSET $SG161063
...
; 78   :     buffer_size_ = uint32_t(wave_format_.nBlockAlign * wave_format_.nSamplesPerSec * 0.4); //400ms
	...
	movsd	QWORD PTR tv436[ebp], xmm0
	fld	QWORD PTR tv436[ebp]
	fnstcw	WORD PTR tv438[ebp]
	movzx	eax, WORD PTR tv438[ebp]
	or	eax, 3072				; 00000c00H
	mov	DWORD PTR tv441[ebp], eax
	fldcw	WORD PTR tv441[ebp]
	fistp	QWORD PTR tv443[ebp]
	fldcw	WORD PTR tv438[ebp]
...
; 92   :   OutputDebugString("dsound init\n");

	mov	esi, esp
	push	OFFSET $SG161051
	call	DWORD PTR __imp__OutputDebugStringA@4
	cmp	esi, esp
	call	__RTC_CheckEsp
