...
; 101  :   SwapBuffers(device_context_);
...
; 96   : void OpenGL::Clear(RGBQUAD color) {
...
; 79   : void OpenGL::SetDisplaySize(int width,int height) {
...
; 80   :   display_width_ = width;

	mov	eax, DWORD PTR _this$[ebp]
	mov	ecx, DWORD PTR _width$[ebp]
	mov	DWORD PTR [eax+8], ecx

; 81   :   display_height_ = height;

	mov	edx, DWORD PTR _this$[ebp]
	mov	eax, DWORD PTR _height$[ebp]
	mov	DWORD PTR [edx+12], eax

; 82   :   glClearColor(0.25f, 0.25f, 0.5f, 0.0f);
...
; 83   :   glViewport(0, 0, display_width_, display_height_);					// Reset The Current Viewport
...
; 85   : 	glLoadIdentity();							// Reset The Projection Matrix
; 86   : 	gluPerspective(45.0f,(GLfloat)display_width_/(GLfloat)display_height_,0.1f,100.0f);
; 87   : 	glMatrixMode(GL_MODELVIEW);						// Select The Modelview Matrix
; 88   :   */
...
; 91   :   glOrtho(0, display_width_, display_height_, 0, 0, 1);
...
; 73   :     wglDeleteContext( render_context_ );
...
; 74   :     ReleaseDC( window_handle_, device_context_ );
...
; 35   : void OpenGL::Initialize(HWND window_handle,int width,int height) {
...
; 40   :   Graphics::Initialize(window_handle,width,height);
...
; 42   :   // get the device context (DC)
; 43   :   device_context_ = GetDC( window_handle_ );
...
; 47   :   }
; 48   :   // set the pixel format for the DC
; 49   :   ZeroMemory( &pfd, sizeof( pfd ) );
...
; 52   :   pfd.dwFlags = PFD_DRAW_TO_WINDOW | PFD_SUPPORT_OPENGL |
; 53   :                 PFD_DOUBLEBUFFER;

	mov	DWORD PTR _pfd$[ebp+4], 37		; 00000025H

; 54   :   pfd.iPixelType = PFD_TYPE_RGBA;

	mov	BYTE PTR _pfd$[ebp+8], 0

; 55   :   pfd.cColorBits = 32;

	mov	BYTE PTR _pfd$[ebp+9], 32		; 00000020H

; 56   :   pfd.cDepthBits = 16;

	mov	BYTE PTR _pfd$[ebp+23], 16		; 00000010H

; 57   :   pfd.iLayerType = PFD_MAIN_PLANE;

	mov	BYTE PTR _pfd$[ebp+26], 0

; 58   :   iFormat = ChoosePixelFormat( device_context_, &pfd );
...
; 59   :   SetPixelFormat( device_context_, iFormat, &pfd );
...
; 60   : 
; 61   :   // create and enable the render context (RC)
; 62   :   render_context_ = wglCreateContext( device_context_ );
...
; 63   :   wglMakeCurrent( device_context_, render_context_ );
...
; 64   :   
; 65   :   //const GLubyte* str = glGetString(GL_EXTENSIONS);
...
; 67   :   SetDisplaySize(width,height);
...
; 27   : OpenGL::OpenGL() :init_(false),device_context_(nullptr),render_context_(nullptr) {
